...
describe("Voting", () => {
    ...
    context("With workflowStatus = VotingSessionEnded", () => {
        ...
        it("prevent from getting winner while status is not VotesTallied.", async () => {
            await expect(votingInstance.connect(voter1).getWinningProposal())
    ...
    context("With workflowStatus = VotesTallied", () => {
        ...
        it("should get the winner by his id", async () => {
            const winner0 = await votingInstance.connect(owner).getWinningProposal();
            const winner1 = await votingInstance.connect(voter1).getWinningProposal();
